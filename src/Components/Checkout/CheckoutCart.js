  ...
  return (
    <div className='px-6 py-5 md:w-1/2 lg:w-1/3 w-full h-full rounded-md shadow-lg'>
      ...
      <div className='flex justify-between items-center p-2 border mt-1 mb-2'>
        <div>{format(new Date(from), 'P')}</div>
        <div>
          <ArrowRightIcon className='h5 w-5' />
        </div>
        <div>{format(new Date(to), 'P')}</div>
      </div>

      <div className='flex border-t border-gray-200 py-2'>
        <span className='text-gray-500'>${price} x {homeData.totalNights} nights</span>
      ...
      <div className='flex border-t border-b mb-6 border-gray-200 py-2'>
        ...
        <span className='ml-auto text-gray-900'>${homeData.totalPrice}</span>
      </div>
    </div>
  )
}
